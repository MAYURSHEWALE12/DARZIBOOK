...
export default function MeasurementList() {
  ...
  const filteredMeasurements = measurements.filter(m => 
    m.customerId?.name?.toLowerCase().includes(search.toLowerCase()) ||
    m.garmentType?.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-[15px]">Manage and view all customer measurements.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-white border-b border-slate-200">
                ...
                <th className="px-6 py-3 text-[11px] font-bold text-[#1e3a8a] uppercase tracking-wider">Garment Type</th>
                <th className="px-6 py-3 text-[11px] font-bold text-[#1e3a8a] uppercase tracking-wider">Date</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredMeasurements.map((m) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate(`/customers/${m.customerId?._id}`)}>
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-[#1e3a8a] font-bold text-sm uppercase shadow-sm border border-white/50">
                        {m.customerId?.name?.charAt(0) || '?'}
                      </div>
                      <span className="text-slate-700 font-semibold group-hover:text-[#1e3a8a] transition-colors">{m.customerId?.name || 'Unknown'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-slate-700 font-medium capitalize">{m.garmentType}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      ...
                      className="text-slate-400 hover:text-[#1e3a8a] transition-colors p-1.5 rounded-lg hover:bg-slate-100 flex items-center justify-center ml-auto"
              ...
              {filteredMeasurements.length === 0 && !loading && (
                <tr>
                  <td colSpan={4} className="py-16 text-center">
                    <div className="flex flex-col items-center justify-center">
                      <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-4 border border-slate-100">
                        <span className="material-symbols-outlined text-[40px] text-slate-300">straighten</span>
                      </div>
                      <p className="text-slate-600 font-semibold text-lg">{search ? "No measurements found" : t('common.noResults')}</p>
                      <p className="text-slate-400 mt-1 max-w-[250px] mx-auto text-sm">
                        {search ? "Try adjusting your search query." : "You haven't added any measurements yet."}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={4} className="py-16 text-center">
                    <div className="flex justify-center items-center gap-3">
                       ...
                       <span className="text-slate-500 font-medium">Loading measurements...</span>
                    </div>
                  </td>
                </tr>
              )}
